...
pub fn main() void {
    ...
    std.debug.print("lo_nibbles 06000101000d0d01060d060101010d00\n", .{});
    std.debug.print("\nhi_nibbles {x}\n", .{std.fmt.fmtSliceHexLower(&@as([16]u8, hi_nibbles))});
    std.debug.print("hi_nibbles 03010902010e0e02030b030209090e01\n", .{});
    ...
    std.debug.print("row_0_07 a1436f6f43b8b86fa1b8a16f6f6fb843\n", .{});
    ...
    std.debug.print("bitmask 08020202024040020808080402024002\n", .{});

    // mask           = [ff|ff|00|ff|ff|00|00|ff|ff|00|ff|ff|00|00|00|ff]
    const mask: v.u8x16 = simd.mm_cmplt_epi8(hi_nibbles, simd.mm_set1_epi8(8));
    const bitsets: v.u8x16 = simd.mm_blendv_epi8(row_0_07, row_8_15, mask);

    std.debug.print("\nmask {s}\n", .{std.fmt.fmtSliceHexLower(&@as([16]u8, mask))});
    std.debug.print("mask ffff00ffff0000ffff00ffff000000ff\n", .{});
    std.debug.print("\nbitsets {s}\n", .{std.fmt.fmtSliceHexLower(&@as([16]u8, bitsets))});

    const tmp: v.u8x16 = simd.mm_and_si128(bitsets, bitmask);
